...
def get_all_next_pages(url):     # inf для Перехода на след. страницы (all_names, all_hats)
    ...
    try:
        ...
        if len_last >= 2:
            ...
            title = browser.title
    if FL_DUMP:
        print(f"get_all_next_pages: in {title}")

    # находим "Связанные статьи" == статьи более глубокого уровня
    for element in browser.find_elements(By.TAG_NAME, "div"):
        try:
            ...
            n_len = len(all_hats)
            if FL_DUMP:
                print(f"Проблема с div class: n_len={n_len}")

    n_len = len(all_hats)
    if FL_DUMP:
        print(f"get_all_next_pages - end: N_hats={n_len} \n")
...
def get_last_page(text_dump,  dict):   # Возвращает url_last, title_last
    ...
    if FL_DUMP:
        print(f"get_last_page - end {text_dump}:  return to  {title_last}" )
...
def get_next_page(n_answer, title_this_page):
    ...
    if FL_DUMP:
        print(f"get_next_page({n_answer}, {title_this_page}")
    next_name = m_all_names[j]
    if FL_DUMP:
        print(f"to new page [{j}] = {next_name}")
        print(f"j_hatNotes={j}")
    ...
    if FL_DUMP:
        ...
        print(f" url_next = {url_next}")
        print(f"get_next_page-end({n_answer}, {title_this_page} goto ==> {next_name}")
    return url_next
...
def print_all_names(hat, m_all_names):
    ...
    for name in m_all_names:
        ...
        i_name += 1
    if FL_DUMP:
        print("print_all_names - end")

def read_user_answer(n_len_in, title):
    # n_len_in = кол  Связанных страниц == "подчиненных" страниц
    if FL_DUMP:
        print(f"\nread_user_answer ({n_len_in}, {title})")
    n_len = n_len_in
    print(f"\n Текущая стр: {title}")
    print("Вы можете:")
    ...
    if FL_DUMP:
        print(f"1) user_answer ={user_answer}")
    if user_answer == "":
        n_answer = 200
    else:
        try:
        ...
        if FL_DUMP:
            print(f"3) n_answer ={n_answer}  n_len={n_len}")
        if n_answer == 100:  # end of work
            return n_answer
        if n_len == 0:
            if not (n_answer -1 or n_answer == 00 or n_answer == 99 ):
                n_answer = 200
                if FL_DUMP:
                    print(f"4)n_len=0  n_answer ={n_answer}")

        elif n_len >0 and (n_answer > (n_len+1) and n_answer != 99) or n_answer <= -2:
            if FL_DUMP:
                print(f"5) n_answer ={n_answer}  n_len={n_len} ==> 100")
            n_answer = 200
    if FL_DUMP:
        print(f"read_user_answer -end ({n_len_in}, {title})")
    return n_answer
...
for i_run in range(MAX_N_RUN):
    ...
    dict = get_all_next_pages(url)   # inf для Перехода на след. страницы
    m_all_names = dict.get("names")
    m_all_hats = dict.get("hats")
    title = dict.get("title")
    n_len = len(m_all_hats)      # кол  Связанных страниц == "подчиненных" страниц
    ...
    if n_answer == -1:  # back: возврат на предыд. стр.
        ...
        if url != None:
            if FL_DUMP:
                print(f"1) Find => url={url}")
            browser.get(url)
            title = browser.title
            fl_url_ok = True
            fl_ok = title.startswith(text_find)
            if FL_DUMP:
                print(f"A1 fl_ok={fl_ok}  fl_url_ok={fl_url_ok}")
            if fl_ok:
                m_last_url.append(url)
                m_last_title.append(title)
        if FL_DUMP:
            print(f"B2 fl_ok={fl_ok}  fl_url_ok={fl_url_ok}")
    ...
    else:   # чтение "подчиненной" странцыи
        url = get_next_page(n_answer, title)  # переход на новую стр
        if FL_DUMP:
            print(f"на след.шаге - выполнить  переход на стр. title = {title}")
        if url != None:
            if FL_DUMP:
                print(f"1) Find => url={url}")
            browser.get(url)
